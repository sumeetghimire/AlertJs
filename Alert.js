...
!function(o,t){"object"==typeof module&&"object"==typeof module.exports?module.exports=t():o.ModalLibrary=t()}("undefined"!=typeof window?window:this,function(){function o(o){let t=window.jQuery.extend({},{title:"Default Title",message:"Default Message",confirmText:"Ok",cancelText:"Cancel",position:"center",backgroundColor:"#ffffff",confirmButtonBackgroundColor:"#007bff",confirmButtonTextColor:"#ffffff",onConfirm:function(){}},o),e=`
        ...
        `;window.jQuery("body").append(e),window.jQuery("#successNotificationModal").modal("show"),setTimeout(function(){window.jQuery("#successNotificationModal").modal("hide").on("hidden.bs.modal",function(){window.jQuery(this).remove()})},t.duration)}function l(o){let t=window.jQuery.extend({},{title:"Dynamic Form",inputs:[],onSubmit:function(o){},onCancel:function(){},submitText:"Submit",cancelText:"Cancel",backgroundColor:"#ffffff",buttonColor:"#007bff",cancelButtonColor:"#6c757d",position:"center"},o),e=o=>{let{type:t,label:e,placeholder:a,id:l,name:n,class:s,options:i}=o;switch(t){case"text":return`<div class="form-group">
                                <label for="${l}">${e}</label>
                                <input type="text" class="form-control ${s||""}" id="${l}" name="${n}" placeholder="${a}" required>
                            </div>`;case"select":return`<div class="form-group">
                                <label for="${l}">${e}</label>
                                <select class="form-control ${s||""}" id="${l}" name="${n}" required>
                                    ${i.map(o=>`<option value="${o.value}">${o.text}</option>`).join("")}
                                </select>
                            </div>`;case"checkbox":return`<div class="form-group form-check">
                                <input type="checkbox" class="form-check-input ${s||""}" id="${l}" name="${n}">
                                <label class="form-check-label" for="${l}">${e}</label>
                            </div>`;case"radio":return`<div class="form-group">
                                <label>${e}</label>
                                ${i.map(o=>`
                                    <div class="form-check">
                                        <input class="form-check-input ${s||""}" type="radio" name="${n}" value="${o.value}" id="${o.id}">
        ...
        `;window.jQuery("body").append(a),window.jQuery("#dynamicFormModal").modal("show");let l=()=>{window.jQuery("#dynamicFormModal").modal("hide").on("hidden.bs.modal",function(){window.jQuery(this).remove()})};window.jQuery("#submitBtn").off("click").on("click",function(){let o={};window.jQuery("#dynamicForm").find("input, select").each(function(){"checkbox"===this.type?o[this.id]=this.checked:o[this.name||this.id]=this.value}),t.onSubmit(o),l()}),window.jQuery("#cancelBtn").off("click").on("click",function(){t.onCancel(),l()})}function n(o){let t=window.jQuery.extend({},{title:"Notification",message:"This is a custom message.",imageSrc:"",backgroundColor:"#ffffff",buttonColor:"#007bff",position:"center"},o),e=`
